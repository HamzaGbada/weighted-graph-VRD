...
def cord_to_CSV():
    ...
    text_cord = {
        "text_units": [
            unit
            for unit_list in train_set.text_units
...
def sroie_to_CSV():
    ...
    text_sroie = {
        "text_units": [
            unit for unit_list in train_set for unit in unit_list[1]["labels"]
        ]
    }
...
def feature_embedding(dataloader_set):
    ...
    if type(dataloader_set).__name__ == "CORD":
        ...
        encoded_dic = {"question": 0, "answer": 1, "header": 2, "other": 3}
    elif type(dataloader_set).__name__ == "SROIE":
    ...
    for doc_index in range(nbr_document):
        ...
        label_encoded_list = [
            encoded_dic[dataloader_set.data[doc_index][1]["labels"][bb_i][1]]
            for bb_i in range(len(dataloader_set.data[doc_index][1]["boxes"]))
        ]
        document_feat = {}
        features = []
        ...
        for bbox_index in range(len(bbox)):
            ...
            data = swapaxes(
                swapaxes(from_numpy(image[ymin:ymax, xmin:xmax]), 0, 2), 1, 2
            ).float()
            ...
            embedding_tensor = stack(
                (spatial_embedding[bbox_index], encoded_data, word_embedding)
            )
...
def edge_builder(data_encoded):
    ...
    for doc_index in range(len(data_encoded)):
        ...
        edge_array_per_doc = remove_duplication(
            cartesian_product(np.arange(nbr_bbox), np.arange(nbr_bbox))
        )
        # src_bbox = [node_src[0] for node_src in edge_array_per_doc]
        # dst_bbox = [node_src[1] for node_src in edge_array_per_doc]
        logger.debug(
            f"edge builded document index after remove duplication: {doc_index}"
        )
...
def edge_metric_calculation(edge_array, node_feature, doc_shape):
    ...
    for edge_link in edge_array:
        ...
        node_src_center = (
            (node_src_feat[0][0] + node_src_feat[0][7]).detach().numpy(),
            node_src_feat[0][1].detach().numpy(),
        )
        node_dst_center = (
            node_dst_feat[0][0].detach().numpy(),
            node_dst_feat[0][1].detach().numpy(),
        )
        ...
        # Spatial Metrics
        euclidean_dist_norm = euclidean(node_src_center, node_dst_center) / euclidean(
            (0, 0), doc_shape
        )
        chebyshev_dist_norm = chebyshev(node_src_center, node_dst_center) / chebyshev(
            (0, 0), doc_shape
        )
        manhattan_dist_norm = cityblock(node_src_center, node_dst_center) / cityblock(
            (0, 0), doc_shape
        )
