...
class GraphConstructor:
    ...
    def __init__(self, dataloader_set) -> None:
        ...
        for doc_index in range(len(embedded_dataset)):
            ...
            doc_shape = (dataloader_set[doc_index][0].shape[1], dataloader_set[doc_index][0].shape[2])

            # The node features
            node_feature = embedded_dataset[doc_index]["boxes_features"]
            top_dict = edge_metric_calculation(edge_array[doc_index], node_feature, doc_shape)
            ...
            logger.debug(f"The number of edge of to_bidirected graph {gph.number_of_edges()}")
            ...
            graph_path = "data/" + data_name + "/train/" + doc_name.split('.', 1)[0] + "_graph.bin"
            if not isTrain:
                graph_path = "data/" + data_name + "/test/" + doc_name.split('.', 1)[0] + "_graph.bin"
