...
class SimpleCNNEncoder(nn.Module):
    def __init__(self, in_channel, out_dim) -> None:
        ...
        self.linear = nn.Sequential(
            nn.LazyLinear(128), nn.ReLU(True), nn.Linear(128, out_dim)
        )
...
class VGG16Encoder(nn.Module):
    def __init__(self, in_channel, out_dim) -> None:
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(
                in_channels=in_channel, out_channels=64, kernel_size=3, padding=1
            ),
            nn.ReLU(True),
...
class ResNet18Encoder(nn.Module):
    def __init__(self, in_channel, residual_block, out_dim) -> None:
        ...
        self.conv1 = nn.Sequential(residual_block(64, 64), residual_block(64, 64))

        self.conv2 = nn.Sequential(
            residual_block(64, 128, residual=True), residual_block(128, 128)
        )

        self.conv3 = nn.Sequential(
            residual_block(128, 256, residual=True), residual_block(256, 256)
        )

        self.conv4 = nn.Sequential(
            residual_block(256, 512, residual=True), residual_block(512, 512)
        )
